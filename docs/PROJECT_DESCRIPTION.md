...
- **Антиплагиат** — автоматическая проверка при сдаче через локальный compare50 (конфиг `plagiarism:` / устаревший `moss:`), кэш исходников и SQLite; флаги для преподавателя без изменения оценки
...
- Локальный движок **compare50** (секция `plagiarism:` или deprecated `moss:`)
- Кэш исходников `plagiarism_cache/` + SQLite `plagiarism.db`
- Фоновая проверка при успешной сдаче (`BackgroundTasks` в `grade_lab`)
- Уведомление: **note** на ячейке оценки (оценка не меняется)
- API списка совпадений: `GET /courses/{course_id}/labs/{lab_id}/plagiarism`
- Пилотный режим: `PLAGIARISM_SHADOW_MODE=true` (результаты пишутся в БД без уведомлений)

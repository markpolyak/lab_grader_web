...
1. **Админка** (основной): страница `/admin/courses/{course_id}/labs`, столбец «Секретная ссылка» - ссылка целиком, кнопка «Скопировать» и состояние окна («откроется 15.10.2026 10:00»). Ссылка доступна с момента появления лабы в конфиге, задолго до открытия: рассылку можно подготовить заранее.

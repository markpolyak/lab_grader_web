...
**Имена джоб из `ci.workflows`, которых нет среди проверок,** просто не учитываются (и пишутся в лог): конфиги перечисляют имена джоб разных поколений шаблонов. Но если **ни одно** имя из конфига не совпало ни с одной проверкой коммита, проверка завершается ошибкой `CI_JOBS_NOT_FOUND` - это устаревший конфиг, а не вина студента, поэтому `x` в таблицу не пишется.

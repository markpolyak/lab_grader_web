...
- `fork` - репозиторий создаётся как настоящий fork шаблона: баннер «forked from», сравнение веток, возможность позже открыть PR из шаблона в репозиторий студента. Сервис автоматически чинит два побочных эффекта форка, которые иначе сломали бы оценивание: включает GitHub Actions (на форках она по умолчанию выключена) и снимает у форка унаследованный флаг «template repository».
